...
~~~
  ...
  4. 코틀린에서는 is를 사용해 변수 타입을 검사한다.
  5. 코틀린에서는 프로그래머 대신 컴파일러가 캐스팅을 해준다. 이걸 스마트 캐스트라고 부른다.
~~~

### 2.3.6 리팩토링: if를 when으로  변경
~~~
  1. 코틀린의 if(a > b) a else b는 자바의 a ? b ? a : b처럼 작동한다.
  2. 코틀린에서는 if가 값을 만들어 내기 때문에 자바와 달리 3항 연산자가 따로 없다.
  3. if의 분기에 식이 하나밖에 없다면 중괄호를 생략해도 된다. 
  4. when식 앞에서 살펴본 값 동등성 검사가 아닌 다른 기능에도 쓸수 있다.
~~~

### 2.3.7 if와 when의 분기에서 블록 사용
~~~
  1. if나 when모두 분기에 블록을 사용할 수 있다. 그런 경우 블록의 마지막 문장이 블록 전체의 결과가 된다.
  2. 식이 본문인 함수는 블록을 본문으로 가질 수 없고 블록이 본문인 함수는 내부에 return문이 반드시 있어야한다.
~~~

## 2.4 대상을 이터레이션 : while 과 for 루트
~~~
  1. for는 자바의 for-each 루프에 해당하는 형태만 존재한다.
  2. 코틀린의 for는 C#과 마찬가지로 for<아이템> in <원소들> 형태를 취한다.
  3. 이런 for루프는 자바에서와 마찬가지로 컬렉션에 대한 이터레이션에 가장 많이 쓰인다.
~~~

### 2.4.1 while 루프
~~~
  1. 코틀린에는 while과 do-while 루프가 있다. 
    1.1 while은 조건이 참인동안 본문을 반복 실행한다.
    1.2 do-while는 맨 처음에 무조건 본문을 한번 실행한 다음, 조건이 참인 동안 본문을 반복 실행한다.
~~~

### 2.4.2 수에 대한 이터레이션: 범위와 수열
~~~
  1. 범위는 기본적으로 두 값으로 이뤄진 구간이다.
  2. 보통은 그 두 값은 정수 등의 숫자 타입의 값이며, ..연산자로 시작 값과 끝값을 연결해서 범위를 만든다.
  3. 정수 범위로 수행할 수 있는 가장 단순한 작업은 범위에 속한 모든 값에 대한 이터레이션이다.
  4. 이런 식으로 어떤 범위에 속한 값을 일정한 순서로 이터레이션하는 경우를 수열이라고 한다.
  5. downTo,step, until이 있다.
    5.1 downTo는 줄어든다.
    5.2 step은 정한수만큼 더하듯이 커진다.
    5.3 until은 마지막을 포함하지않는다.
~~~

### 2.4.3 맵에 대한 이터레이션
~~~
  1. 맵에 사용해던 구조 분해 구문을 맵이 아닌 컬렉션에도 활용할 수 있다.
  2. 그런 구조 분해 구문을 사용하면 원소의 현재 인덱스를 유지하면서 컬렉션을 이터레이션 할수있다.
  3. 인덱스를 저장하기 위한 변수를 별도로 선어하고 루프에서 매번 그 변수를 증가시킬 필요가 없다.
  4. 컬렉션이나 범위에 대해 in 키워들르 사용하는 방법을 살펴봤다.
  5. 한편 어떤 값이 범위나 컬렉션에 들어있는지 알고 싶을 때도 in을 사용한다.
~~~
...
~~~
  ...
  1. in 연산자를 사용해 어떤 값이 범위에 속하는지 검사할 수 있다. 
     반대로 !in을 사용하면 어떤 값이 범위에 속하지 않는지 검사할 수 있다.
  2. 이렇게 어떤 문자가 글자인지 검사하는 방법은 간다해 보인다.
  3. 내부적으로도 교묘한 부분은 전혀 없다.
  4. 이렇게 코드를 작성해도 여전히 문자의 코드가 범위의 첫번째 글자의 코드와 마지막 글자의 코드와 마지막 글자의 코드 사이에 있는지를 비교한다.
  5. 하지만 그런 비교 로직 표준 라이브러리의 범위 클래스 구현 안에 깔끔하게 감춰져 있다.
  6. java와 kotlin 사이의 모든 문자열을 이터레이션할 수는 없다.
~~~

## 2.5 코틀린의 예외 처리
~~~
  1. 코틀린의 예회 처리는 자바나 다른 언어의 예회 처리와 비슷하다.
  2. 함수는 정상적으로 종료할 수 있지만 오류가 발생하면 예회를 던질수 있다.
  3. 함수를 호출하는 쪽에서는 그 예외를 잡아 처리할 수 있다.
  4. 발생한 예외를 함수 호출 단에서 처리하지않으면 함수 호출 스택을 거슬러 올라가면서 예외를 처리하는 부분이 나올때까지 예외를 다시 던진다.
~~~
...
~~~
  ...
  1. 파일에서 각 줄을 읽어 수로 변환하되 그줄이 올바른 수 형태가 아니면 null을 반환하는 다음 예제에서 그 세 가지 요소를 모두 볼 수 있다.
  2. 자바 코드와 가장 큰 차이는 throws(이 경우 s가 붙어 있다) 절이 코드에 없다는 점이다.
  3. 어떤 함수가 던질 가능성이 잇는 예외나 그 함수가 호출한 
     다른 함수에서 발생할 수 있는 예외를 모두 catch로 처리하지 않은 예외는 throws 절에 명시해야 한다.
  4. 코틀린에서는 함수가 던지는 예외를 지정하지 않고 발생한 예외를 잡아내도 되고 잡아내지 않아도 된다.
~~~
